        ...
        files.forEach(file => {
            ...
            if (stat.isFile() && (extension === '.md' || extension === '.markdown')) {
                ...
                if (
                    ...
                    type === 'aggdocs' &&
                    fs.existsSync(path.join(cwd, './src/pages/aggdocumentation'))
                ) {
                    const aggDocumentation = require(path.join(cwd, './src/pages/aggdocumentation'))
                        .default;
                    ejs.renderFile(
                        path.join(cwd, './template.ejs'),
                        {
                            ...
                            description: result.meta.description || fileInfo.name,
                            rootPath: window.rootPath,
                            page: 'aggDocumentation',
                            __html: ReactDOMServer.renderToString(
                                React.createElement(aggDocumentation, {
                                    lang,
                                    __html: result.__html,
                                    meta: result.meta
                                }, null)
...
const generateJSONFile = (env, cwd) => {
    ...
    generate(env, cwd, 'aggdocs', 'zh-cn', docsData);
    generate(env, cwd, 'aggdocs', 'en-us', docsData);
